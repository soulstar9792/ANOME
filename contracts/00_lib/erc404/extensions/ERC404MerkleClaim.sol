...
import {MerkleProof} from "../../zeppelin/utils/cryptography/MerkleProof.sol";
...
abstract contract ERC404MerkleClaim is IERC404MerkleClaim {
    ...
    function verifyProof(bytes32[] memory proof_, address claimer_, uint256 value_) public view returns (bool) {
        bytes32 leaf = keccak256(bytes.concat(keccak256(abi.encode(claimer_, value_))));
    ...
    function airdropMint(bytes32[] memory proof_, uint256 value_) public virtual whenAirdropIsOpen {
    ...
    function _validateAndRecordAirdropClaim(bytes32[] memory proof_, address claimer_, uint256 value_) internal {
